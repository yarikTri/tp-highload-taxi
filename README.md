...
Хранение всех логов 3 месяца, год+ важных (ошибки)
БД отдельно от бэкенда
Retry policy в межсервисных запросах
Резервирование аппаратных ресурсов с запасом
Политика выдачи квот сервисам
Политика антифрода
Снапшоты/резервные копии PostgreSQL, S3
SRE-инженеры
Несколько независимых балансеров
Healthcheck'и сервисов
Rate limiter клиентских запросов по ip
Кэши для оптимизаций
Множественная репликация
Смена мастер-реплики при падении ЦОДа
