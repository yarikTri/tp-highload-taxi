...
Так как в случае Яндекс.Такси 67% заказов приходится на российский рынок[^1], то ЦОДы размещать стоит в первую очередь в наиболее густонаселённых округах РФ (ЦФО, ПФО, ЮФО), не забыв при этом про отдалённые регионы (СФО, ДВФО).

При выборе населённого пункта для размещения ЦОДа стоит также обращать внимание на стоимость электроэнергии и географическую доступность - для более простой поддержки, обслуживания и проведения технических работ на серверах.

// TODO: Дополнить про СНГ и EMEA

### Методы глобальной балансировки

// TODO: Описать преимущества Latency-based DNS
